...
export const handleChatMessage = async (bot: any, msg: Message, mode: string) => {
  ...
  const sendOptions = { reply_to_message_id: msg.message_id };
  ...
  console.log(`\n\n--------from: ${isPrivate ? name : msg.chat.title}, chat_id: ${chatId}, message_id: ${msg.message_id}, mode: ${mode}, time: ${moment(Date.now()).utcOffset('+0700').format('DD/MM/yyyy HH:mm')}`);
  ...
  const res: Message = stickerFileId ? await bot.sendSticker(chatId, stickerFileId, sendOptions) : await bot.sendMessage(chatId, replyContent, sendOptions);
